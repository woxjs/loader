...
module.exports = class Helper {
  ...
  customRender(name, prefix, reg, callback) {
    ...
    this.app.addCompiler(({ dir, isRoot, isDev }) => {
      ...
      if (fs.existsSync(fileDir)) return this.staticRender(name, fileDir, reg, callback);
      if (this.app.watch && (isRoot || isDev)) this.addRenderWatcher(name, fileDir, reg, callback);
    });
  }
  ...
  SERVICE_CONTROLLER() {
    this.customRender('controller', './app/controller', /\.js(x)?$/);
  }
